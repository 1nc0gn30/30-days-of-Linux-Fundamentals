...
int main() {
    printf("\033[H\033[J");
    printf(BOLD_COLOR CYAN_COLOR "Day 27 - Dinner: Practical Docker Labs\n" RESET_COLOR);
    getchar();

    Lesson lessons[] = {
        {
            "Run Your First Container",
            "Pull a Docker image and run your first container.",
            "docker run hello-world",
            "This command pulls the 'hello-world' image from Docker Hub and runs a simple container to confirm your Docker installation is working.",
        ...
        },
        {
            ...
            "Write a Dockerfile in your directory, then use 'docker build' to create a custom image. The '-t' flag tags the image with a name.",
            {"https://docs.docker.com/engine/reference/builder/", NULL, NULL, NULL, NULL}
        },
        {
            "Manage Multiple Containers",
            "Practice listing, stopping, and removing containers.",
            "docker ps -a\ndocker stop <id>\ndocker rm <id>",
            "List containers with 'docker ps -a'. Use 'docker stop' to halt containers and 'docker rm' to delete them. Helps clean up your Docker environment.",
    ...
    Question questions[] = {
        {
            "What command runs a basic Docker container to test installation?",
            {"docker image hello-world", "docker hello-world", "docker run hello-world", "docker exec hello-world"},
            "docker run hello-world"
        },
        {
            "Which file is used to build custom Docker images?",
            {"Dockerconfig", "Dockerfile", "Imagefile", "Containerfile"},
            "Dockerfile"
        },
        {
            "What does 'docker ps -a' show?",
            {"Only running containers", "Only stopped containers", "All containers", "Container logs"},
            "All containers"
        },
        {
            "How do you remove a container by ID?",
            {"docker remove", "docker rm", "docker stop", "docker clean"},
            "docker rm"
        }
    };

    printf(BOLD_COLOR GREEN_COLOR "\nDinner Quiz – Docker Hands-on Practice:\n" RESET_COLOR);
    for (int i = 0; i < sizeof(questions)/sizeof(Question); i++) {
        display_question(&questions[i]);
    }

    printf(BOLD_COLOR CYAN_COLOR "\nDinner complete. You're now practicing Docker like a pro!\n" RESET_COLOR);
    return 0;
}

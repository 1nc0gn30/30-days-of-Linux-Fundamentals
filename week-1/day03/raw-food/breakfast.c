...
const char *questions[][2] = {
    {"1. What command can create a new file with initial content?", "echo 'Initial content' > file.txt"},
    {"2. How do you recursively copy a directory and its contents?", "cp -r source/ destination/"},
    {"3. What command is used to move a file to a different directory?", "mv file.txt /path/to/destination/"},
    {"4. How can you rename a file in the current directory?", "mv oldname.txt newname.txt"},
    {"5. How can you switch to the previous directory you were in?", "cd -"}
};
...
int main() {
    ...
    for (int i = 0; i < sizeof(lessons) / sizeof(lessons[0]); i++) {
        display_lesson(lessons[i][0], lessons[i][1], lessons[i][2]);
    }
    ...
    for (int i = 0; i < sizeof(questions) / sizeof(questions[0]); i++) {
        ask_question(questions[i][0], questions[i][1]);
    }

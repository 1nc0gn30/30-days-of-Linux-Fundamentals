...
int main() {
    ...
    ask_question(
        "What command would you use to view the contents of a file one page at a time?",
        "more"
    );

    ask_question(
        "Which text editor is known for its advanced features but has a steeper learning curve?",
        "vim"
    );

    ask_question(
        "How do you perform a case-insensitive search for a pattern in a file using `grep`?",
        "grep -i 'pattern' file.txt"
    );

    ask_question(
        "What command can you use to replace 'old' with 'new' in a file using `sed`?",
